...
router.post('/addnote', fetchuser, [
    ...
    try {
        ...
        const note = new Note({
            title, desc, tag, mark, user: req.user.id //Here id with (req.user.id) :- where user is foreign key and it comes from user collection
        })
...
// Route 5: Get all notes of logged in user: GET "/api/notes/fetchadmin" --login required
...
// Route 6: Get all notes of logged in user: GET "/api/notes/fetchadmin" --login required

router.delete('/deletenoteadmin/:id', async (req, res) => {
    try {
        // Deleting note by id
        note = await Note.findByIdAndDelete(req.params.id)
        res.json({ "Success": "Note has been deleted", note });
